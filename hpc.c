...
* 
* Notes: 
* - To view initial unsorted array uncomment line A * - To view local arrays of processes before sorting uncomment  line B 
* - To view final sorted array uncomment line C 
* 
*/ 
#include <mpi.h> // MPI 
#include <stdio.h> // printf 
#include <stdlib.h> // malloc, free, rand(), srand() #include <time.h> // time for random generator 
#include <math.h> // log2 
...
int* merge(int half1[], int half2[], int mergeResult[], int size); int* mergeSort(int height, int id, int localArray[], int size,  MPI_Comm comm, int globalArray[]); 
/*------------------------------------------------------------------ -
...
void getInput(int argc, char* argv[], int id, int numProcs, int*  arraySize){ 
 if (id == 0){ 
 if (id % 2 != 0){ 
fprintf(stderr, "usage: mpirun -n <p> %s <size of  
array> \n", argv[0]); 
 fflush(stderr); 
 *arraySize = -1; 
 } else if (argc != 2){ 
 ...
 fflush(stderr); 
 *arraySize = -1; 
 } else if ((atoi(argv[1])) % numProcs != 0) {  fprintf(stderr, "size of array must be divisible by  number of processes \n"); 
 fflush(stderr); 
 *arraySize = -1; 
} else { 
 *arraySize = atoi(argv[1]); 
 ...
 MPI_Bcast(arraySize, 1, MPI_INT, 0, MPI_COMM_WORLD);  // negative arraySize ends the program 
 if (*arraySize <= 0) { 
 MPI_Finalize(); 
 exit(-1); 
 } 
...
* id, rank of the current process 
*/ 
void fillArray(int array[], int arraySize, int id) { int i; 
...
/*------------------------------------------------------------------ - 
* Function: printList 
* Purpose: Prints the contents of a given list of a process * Params: id, rank of the current process 
...
* the first int is less than, equal, or greater than * the second. Used by qsort. 
*/ 
int compare(const void* a_p, const void* b_p) { 
 int a = *((int*)a_p); 
 int b = *((int*)b_p); 
...
/*------------------------------------------------------------------ - 
* Function: merge 
* Purpose: Merges half1 array and half2 array into mergeResult * Params: half1, first half of array to merge 
* half2, second half of array to merge * mergeResult, array to store merged result * size, size of half1 and half2 
*/ 
int* merge(int half1[], int half2[], int mergeResult[], int size){  int ai, bi, ci; 
 ai = bi = ci = 0; 
...
/*------------------------------------------------------------------ - 
* Function: mergeSort 
* Purpose: implements merge sort: merges sorted arrays from  * processes until we have a single array containing  all  
* integers in sorted order 
* Params: height, height of merge sort tree 
* id, rank of the current process * localArray, local array containing integers of  current process 
* size, size of localArray on current process * comm, MPI communicator 
...
int* mergeSort(int height, int id, int localArray[], int size,  MPI_Comm comm, int globalArray[]){ 
 ...
 // allocate memory and receive array of right child  half2 = (int*) malloc (size * sizeof(int));  MPI_Recv(half2, size, MPI_INT, rightChild, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
 // allocate memory for result of merge  mergeResult = (int*) malloc (size * 2 * sizeof(int));  // merge half1 and half2 into mergeResult  mergeResult = merge(half1, half2, mergeResult, size);  // reassign half1 to merge result 
 half1 = mergeResult; 
...
int main(int argc, char** argv) { 
 int numProcs, id, globalArraySize, localArraySize, height;  int *localArray, *globalArray; 
 ...
 if (id==0){ 
globalArray = (int*) malloc (globalArraySize *  
sizeof(int)); 
fillArray(globalArray, globalArraySize, id); 
//printList(id, "UNSORTED ARRAY", globalArray,  
globalArraySize); // Line A 
 ...
 localArray = (int*) malloc (localArraySize * sizeof(int));  MPI_Scatter(globalArray, localArraySize, MPI_INT, localArray,  localArraySize, MPI_INT, 0, MPI_COMM_WORLD); 
...
globalArray = mergeSort(height, id, localArray,  
localArraySize, MPI_COMM_WORLD, globalArray); 
...
printf("Process #%d of %d on %s took %f seconds \n",  id, numProcs, myHostName, processTotalTime); 
 ...
 if (id == 0) { 
//printList(0, "FINAL SORTED ARRAY", globalArray,  
